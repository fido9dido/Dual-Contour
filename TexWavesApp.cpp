...
private:
	...
	ComPtr<ID3D12DescriptorHeap> m_SamplerDescrHeap = nullptr;
...
void TexWavesApp::Draw(const GameTimer& gt)
{
	...
	ID3D12DescriptorHeap* DescHeaps[] = { mSrvDescHeap.Get() };
...
void TexWavesApp::BuildDescHeaps()
{
	//
	// Create the SRV heap.
	//
	...
	ThrowIfFailed(md3dDevice->CreateDescriptorHeap(&srvHeapDesc, IID_PPV_ARGS(&mSrvDescHeap)));
 ...
 void TexWavesApp::BuildLandGeometry()
{
	...
	GeometryGenerator::MeshData Cube = geoGen.CreateCubeGrid(32, 32, 32);
	...
	{
		XMStoreFloat3(&vertices[k].Pos, Cube.Vertices[i].pos);
		XMStoreFloat3(&vertices[k].Normal, Cube.Vertices[i].normal);
		vertices[k].TexC = Cube.Vertices[i].texCoord;

	}

	std::vector<std::uint32_t> indices;

	indices.insert(indices.end(), std::begin(Cube.Indices32), std::end(Cube.Indices32));
...
void TexWavesApp::BuildMaterials()
{
	...
	mMaterials["crate"] = std::move(crate0);
}
...
void TexWavesApp::BuildRenderItems()
{
 ...
 //   mWavesRitem = wavesRitem.get();
	...
	gridRitem->Mat = mMaterials["crate"].get();
